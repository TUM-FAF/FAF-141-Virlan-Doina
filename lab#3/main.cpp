...
COLORREF borderColor = RGB(25, 144, 82);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow)
{
    ...
    wnd.hbrBackground = (HBRUSH)CreateSolidBrush(RGB(0,0,255));
    ...
    hwnd = CreateWindowEx(
        ...
        800, 545,                                                               // Set all the dimensions to default value
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    int xFillPreview   = 570;
    int yFillPreview   = 430;
    int xStrokePreview = 570;
    int yStrokePreview = 455;


    HDC hdcMem;
    BITMAP bitmap;
    HBITMAP hbmppaint_brushImage = NULL;
    hbmppaint_brushImage = (HBITMAP)LoadImage(hInst, "untitled.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    GetObject(hbmppaint_brushImage, sizeof(bitmap), &bitmap);

    static RECT drawingArea = {0, 0, 795, 400};
    ...
    static RECT gradientRect = {745, 420, 790, 480};
    ...
    switch(msg)
    {
        ...
        case WM_PAINT:
            ...
            SelectObject(hdcMem, hbmppaint_brushImage);
            BitBlt(hdc, 0, 400, bitmap.bmWidth, bitmap.bmHeight, hdcMem, 0, 0, SRCCOPY);
            DeleteDC(hdcMem);
...
void updateColorControls(HDC hdc, COLORREF rgb, int xLeft, int yTop)
{
    ...
    HPEN hPen = CreatePen(PS_INSIDEFRAME, 2, RGB(125, 440, 140));
