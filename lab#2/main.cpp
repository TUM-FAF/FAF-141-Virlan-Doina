...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR szCmdLine, int iCmdShow)
{
    ...
    wnd.hbrBackground = CreateSolidBrush(RGB(222, 231, 249));
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    PAINTSTRUCT ps;
    HDC hdc;
    LPSTR title = TEXT("  A new message :");
    LRESULT textSize;
    HBRUSH color, hBrushStatic;
    char * message = new char[100];
    char * placeholder = new char[26];
    placeholder = "Type here the new message ...";
    int screenW;
    int screenH;
    char * itemCnt;
    ...
    switch(msg)
    {
        case WM_CREATE:
            ...
            hwndAddButton = CreateWindowEx(
                ...
                TEXT("Add to the listbox"),                                   // the caption of the button
            ...
            switch (LOWORD(wParam))
            {
                /*case IDC_COUNT_BUTTON:
                    if(HIWORD(wParam) == BN_CLICKED)
                    {
                        ...
                        sprintf(message, "There are %d message in the list.", items);
...
*/
        ...
        case WM_CTLCOLOREDIT:
            switch(GetDlgCtrlID((HWND)lParam))
            {
                case IDC_TEXT_INPUT:
                {
                    ...
                    {
                        color = CreateSolidBrush(RGB(255, 255, 255));
                        SetTextColor(hdc, RGB(160, 160, 160));                  // Text color
                        SetBkMode(hdc, TRANSPARENT);                            // EditBox Backround Mode
                        SetBkColor(hdc,(LONG)color);                            // Backround color for EditBox
                    }
        ...
        case WM_PAINT:
            ...
            SetTextColor(hdc, RGB(152, 0, 20));                                 // Setting the text color to a dark red
            ...
            if(index != -1)
            {
                ...
                SetWindowText(hwndCurrentTask, "No message selected");
        ...
        case WM_CLOSE:
            if(MessageBox(hwnd, "Are you sure that you want to close this option ?", "Lab#2", MB_OKCANCEL | MB_ICONWARNING) == IDOK)
                DestroyWindow(hwnd);
            break;
        case WM_DESTROY:
            PostQuitMessage(0);
